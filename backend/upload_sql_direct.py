"""
Direct SQL Upload to Supabase using existing credentials
"""
import os
import psycopg2
...
def upload_all_sql_scripts():
    """Upload all Adhi Compliance SQL scripts"""
...
"""
        ...
        if not execute_sql_script(conn, enum_script, "Enum Types"):
            return False
...
"""
        ...
        if not execute_sql_script(conn, tables_script, "Tables Creation"):
            return False
        
        # Script 3: Insert demo data
        data_script = """
-- 1. Service Provider
...
"""
        ...
        if not execute_sql_script(conn, data_script, "Demo Data"):
            return False
        ...
        print(f"[SUCCESS] Setup verification:")
        print(f"  - AI Systems: {ai_count}")
        ...
        print("\n[COMPLETE] All SQL scripts uploaded successfully!")
        return True
...
if __name__ == "__main__":
    print("=" * 60)
    print("ADHI COMPLIANCE - DIRECT SQL UPLOAD")
    print("=" * 60)
    
    if upload_all_sql_scripts():
        print("\nðŸŽ‰ SUCCESS! Database setup complete!")
        print("\nNext steps:")
        print("1. Create auth user in Supabase Dashboard:")
        print("   Authentication > Users > Add user")
        print("   Email: ganeshkhovalan2203@gmail.com")
        print("   Password: [REDACTED_PASSWORD]")
        print("2. Test: py test_auth_simple.py")
    else:
        print("\nâŒ Setup failed. Check the error messages above.")
